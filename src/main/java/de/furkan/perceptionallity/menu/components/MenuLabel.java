...
public class MenuLabel extends MenuComponent {
  ...
  public MenuLabel(int x, int y, String text, float size, Color color) {
    ...
    Rectangle2D rectangle2D = rawComponent.getFontMetrics(rawComponent.getFont()).getStringBounds(text,rawComponent.getGraphics());
    Dimension dimension = new Dimension(Math.toIntExact(Math.round(rectangle2D.getWidth()+1)), Math.toIntExact(Math.round(rectangle2D.getHeight())));
